...
main = do
  results <- mapM Q.quickCheckResult
    ...
    ,Q.label "class_idb" prop_class_idb
    ,Q.label "class_qdb" prop_class_qdb
    ,Q.label "class_jdb" prop_class_jdb
    ,Q.label "class_wdb" prop_class_wdb]
  if any (not . isSuccess) results then exitFailure else exitSuccess

prop_idb :: Int64 -> Property
prop_idb key = QM.monadicIO $ do
  val <- QM.pick asciis
...
prop_class_idb :: Int64 -> String -> Property
prop_class_idb  key val = QM.monadicIO $ do
  QM.pre $ validKS key val
...
prop_class_qdb :: Int64 -> Property
prop_class_qdb key = QM.monadicIO $ do
  val <- QM.pick asciis
  QM.pre $ validKS key val
...
getTDDir baseName = do
  path <- (</> "td" </> baseName) `fmap` getTemporaryDirectory
  unless (hasExtension baseName) (createDirectoryIfMissing True path)
  return path
